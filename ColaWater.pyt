...
class CalculateFacilityIdentifiers(object):
    ...
    def getParameterInfo(self) -> list[arcpy.Parameter]:
        ...
        initials = arcpy.Parameter(displayName="Initials", name="initials", datatype="GPString", parameterType="Required", direction="Input")
        # interval to increment the start each loop 
        interval = arcpy.Parameter(displayName="Global Interval", name="interval", datatype="GPLong", parameterType="Required", direction="Input")
        interval.value = 2
    ...
    def execute(self, parameters, messages) -> None:
        ...
        # see the function docstrings for info on the rest of the execution
                if layer and start:
                    self.update_info(messages, f"Calculating facility identifiers for '{layer_name}'..." )
                    new_fid = self.calculate_facility_identifiers(layer, layer_short_name, initials, start, interval, messages) 
                    self.update_info(messages, f"'{layer_name}' finished.")

                    new_starts.update(layer_short_name, new_fid)
                else:
                    self.update_warn(messages, f"Layer or start value omitted: skipping '{p.displayName}'")

        new_starts.post(messages)


    def calculate_facility_identifiers(self, layer, layer_short_name: str, initials: str, start: int, interval: int, messages: list) -> str:
        ...
        try: 
            ...
            self.update_err(messages, "If you see an error that says 'RuntimeError: Cannot acquire a lock.', close the attribute tables of the layers for which you are trying to calculate facility identifiers. If that still does not work, go find whomever wrote this tool and ask them about it.")
